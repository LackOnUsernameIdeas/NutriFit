...
export default function MealPlannerForm(props: {
  ...
  const generatePlanWithGemini = async () => {
    try {
      ...
      Съобрази се с начина на приготвяне и рецептите вече като имаш в предвид какво НЕ ТРЯБВА да се включва.
      Имената на храните трябва да са адекватно преведени и написани, така че да са съществуващи храни. 
      Моля те, съобрази се с type-овете и давай числа, където трябва! Не изпускай кавички, запетаи или каквото и да е, дай нещата точно както ги искам! НЕ ДОБАВЯЙ ИЗЛИШНИ НЕЩА! Използвай double quotes, а не single quotes!
      Форматирай ЦЯЛАТА информация в JSON точно така: '{
      breakfast':{
        'main':{'name':'string','totals':{'calories':number,'protein':number,'fat':number,'carbohydrates':number,'grams':number}, 'ingredients':['quantity ingredient','quantity ingredient','quantity ingredient','quantity ingredient','quantity ingredient'...], 'instructions':['1.string','2.string','3.string','4.string'...], 'recipeQuantity': number (in grams)}
      },
      ...
      Грамажът на ястията е ЗАДЪЛЖИТЕЛНА стойност, която НЕ трябва да е повече от 500 грама. Стойностите на 'totals' трябва ЗАДЪЛЖИТЕЛНО да са number, а не string! Съобрази се с подадените калории: ${userPreferences.Calories} и подадените мазнини: ${userPreferences.Fat} и НЕ СЕ ОТДАЛЕЧАВАЙ ОТ ТЯХ! Не включвай грамажа в името на ястието, а го дай САМО като стойност в totals. Направи менюто по начин, който всеки път когато се генерира да бъде уникално и ново от минали пъти. Също избягвай често препоръчани от теб ястия като овесени ядки и сьомга. ВИНАГИ СЕ СЪОБРАЗЯВАЙ СЪС ПОДАДЕНИТЕ ЛИМИТИ! Не се отклонявай от тях! ВСИЧКО ТРЯБВА ДА Е НА БЪЛГАРСКИ! Винаги давай валидна JSON структура!!`;
