...
export default function MealPlannerForm(props: {
  ...
  const generatePlanWithGemini = async () => {
    try {
      ...
      const excludeLine =
        userPreferences.Exclude.length > 0
          ? `Добави всички останали условия към менюто, но се увери, че избягваш стриктно да включваш следните елементи в менюто на храните: ${userPreferences.Exclude}. Съобрази се с начина на приготвяне и рецептите вече като имаш в предвид какво НЕ ТРЯБВА да се включва.`
          : "";

      const prompt = `Създайте ми дневно меню с ниско съдържание на мазнини, включващо едно ястие за закуска, три за обяд (третото трябва да е десерт) и две за вечеря (второто да бъде десерт). Направи менюто по начин, който всеки път когато се генерира да бъде уникално и ново от минали пъти. 
      Избирай САМО И ЕДИНСТВЕНО традиционни храни от следните кухни: ${promptCuisine}, но ВИНАГИ ПРЕВЕЖДАЙ ВСИЧКО НА БЪЛГАРСКИ ЕЗИК! Когато избираш храни за менюто, пресметни сумата на калории, протеин, въглехидрати и мазнини и усигури, че стриктно спазваш следните лимити: ${userPreferences.Calories} калории, ${userPreferences.Protein} грама протеин, ${userPreferences.Fat} грама мазнини и ${userPreferences.Carbohydrates} грама въглехидрати. Ако менюто има сравнително по-малко или по-голямо отклонение от лимитие, преработи менюто така че да съответстват на тях. 
      НЕ Предоставяйте храни, които накрая имат отклонение в количеството калории, въглехидрати, протеин и мазнини в сравнение с посочените общи лимити: (${userPreferences.Calories} калории, ${userPreferences.Protein} грама протеин, ${userPreferences.Fat} грама мазнини и ${userPreferences.Carbohydrates} грама въглехидрати) и НИКОГА, АБСОЛЮТНО НИКОГА не давай хранителен план, който да се отклонеява от лимитите на потребителя с повече от - 100 калории, 10 грама протеини, 20 грама въглехидрати, 10 грама мазнини.
      Включвай само съществуващи в реалния свят храни в хранителния план. Когато включваш храни в хранителният режим, давай винаги различни храни от миналият ти отговор! Предоставете точни мерки и точни стойности за калории, протеин, въглехидрати и мазнини за закуска, обяд и вечеря. 
      Подсигури рецепти за приготвянето на храните и нужните продукти(съставки) към всяко едно ястие. Направи рецептите и съставките, така че да се получи накрая точното количество, което ще се яде, не повече от това. Рецептите и съставките трябва винаги да са на български!
      Имената на храните трябва да бъдат адекватно преведени на български език!
      ${excludeLine}
      Съобрази се с type-овете и давай числа, където трябва! Не изпускай кавички, запетаи или каквото и да е, дай нещата точно както ги искам във валиден JSON! НЕ ДОБАВЯЙ ИЗЛИШНИ НЕЩА! Използвай double quotes, а не single quotes!
      Форматирай ЦЯЛАТА информация в JSON точно така: '{
      breakfast':{
        'main':{'name':'string','totals':{'calories':number,'protein':number,'fat':number,'carbohydrates':number,'grams':number}, 'ingredients':['quantity ingredient','quantity ingredient','quantity ingredient','quantity ingredient','quantity ingredient'...], 'instructions':['1.string','2.string','3.string','4.string'...], 'recipeQuantity': number (in grams)}
      },
      ...
      Грамажът на ястията е ЗАДЪЛЖИТЕЛНА стойност, която НЕ трябва да е повече от 500 грама. Стойностите на 'totals' трябва ЗАДЪЛЖИТЕЛНО да са number, а не string! Съобрази се с подадените калории: ${userPreferences.Calories} и подадените мазнини: ${userPreferences.Fat} и НЕ СЕ ОТДАЛЕЧАВАЙ ОТ ТЯХ! Не включвай грамажа в името на ястието, а го дай САМО като стойност в totals. Също избягвай често препоръчани от теб ястия като овесени ядки и сьомга. ВИНАГИ СЕ СЪОБРАЗЯВАЙ СЪС ПОДАДЕНИТЕ ЛИМИТИ! Не се отклонявай от тях! ВСИЧКО ТРЯБВА ДА Е НА БЪЛГАРСКИ! Винаги давай валидна JSON структура!!`;
