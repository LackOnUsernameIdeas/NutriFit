...
export default function MealPlannerForm(props: {
  ...
  const generatePlanWithGemini = async () => {
    try {
      ...
      Грамажът на ястията е ЗАДЪЛЖИТЕЛНА стойност, която НЕ трябва да е повече от 500 грама. Стойностите на 'totals' трябва ЗАДЪЛЖИТЕЛНО да са number, а не string! Съобрази се с подадените калории: ${userPreferences.Calories} и подадените мазнини: ${userPreferences.Fat} и НЕ СЕ ОТДАЛЕЧАВАЙ ОТ ТЯХ! Не включвай грамажа в името на ястието, а го дай САМО като стойност в totals. Направи менюто по начин, който всеки път когато се генерира да бъде уникално и ново от минали пъти. Също избягвай често препоръчани от теб ястия като овесени ядки и сьомга. ВИНАГИ СЕ СЪОБРАЗЯВАЙ СЪС ПОДАДЕНИТЕ ЛИМИТИ! Не се отклонявай от тях! ВСИЧКО ТРЯБВА ДА Е НА БЪЛГАРСКИ!`;
