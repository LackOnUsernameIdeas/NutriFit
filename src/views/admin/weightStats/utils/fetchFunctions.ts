...
function translateBMIHealthToBulgarian(englishHealth: string) {
    const bulgarianTranslations = {
        'Severe Thinness': 'Сериозна Художественост',
        'Mild Thinness': 'Лека Художественост',
        'Normal': 'Нормално',
        'Healthy weight': 'Здравословно weight',
        'Overweight': 'Наднормено weight',
...
export const fetchBMIData = async (
    ...
    try {
        fetch(`https://fitness-calculator.p.rapidapi.com/bmi?age=${age}&weight=${weight}&height=${height}`, {
            method: "GET",
            headers: headers
        })
        .then((res) => res.json())
...
export const fetchPerfectWeightData = async (
    ...
    try {
        fetch(`https://fitness-calculator.p.rapidapi.com/idealweight?gender=${gender}&height=${height}`, {
            method: "GET",
            headers: headers
        })
        .then((res) => res.json())
...
export const fetchBodyFatAndLeanMassData = async (
    ...
    try {
        fetch(`https://fitness-calculator.p.rapidapi.com/bodyfat?age=${age}&gender=${gender}&weight=${weight}&height=${height}&neck=${neck}&waist=${waist}&hip=${hip}`, {
            method: "GET",
            headers: headers
        })
        .then((res) => res.json())
...
export const fetchCaloriesForActivityLevels = async (
    ...
    height: number,
    weight: number,
    setDailyCaloryRequirements: React.Dispatch<React.SetStateAction<DailyCaloryRequirements[]>>
) => {
    try {
        const requests = [];
